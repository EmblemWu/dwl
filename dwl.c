...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		drwl_text(m->drw, x, -under_padding, w, m->b.height + under_padding, m->lrpad / 2, tags[i], urg & 1 << i);
		x += w;
	}
	w = TEXTW(m, m->ltsymbol);
	drwl_setscheme(m->drw, colors[SchemeNorm]);
	x = drwl_text(m->drw, x, -under_padding, w, m->b.height + under_padding, m->lrpad / 2, m->ltsymbol, 0);

	if ((w = m->b.width - tw - x) > m->b.height) {
		if (n > 0) {
		    Client *sel = focustop(selmon);
			tw = TEXTW(selmon, client_get_title(sel)) + m->lrpad;
			mw = (tw >= w || n == 1) ? 0 : (w - tw) / (n - 1);

			i = 0;
			wl_list_for_each(c, &clients, link) {
				if (!VISIBLEON(c, selmon) || c == sel)
					continue;
			...
			wl_list_for_each(c, &clients, link) {
				if (!VISIBLEON(c, selmon))
					continue;
				tw = MIN(sel == c ? w : mw, TEXTW(selmon, client_get_title(c)));

				drwl_setscheme(m->drw, colors[m == selmon && sel == c ? SchemeSel : SchemeNorm]);
				if (tw > m->lrpad / 2)
					drwl_text(m->drw, x, -under_padding, tw, m->b.height + under_padding, m->lrpad / 2, client_get_title(c), 0);
				if (c->isfloating)
